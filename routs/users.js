...
router.delete('/', userSchema.auhorizedUserSchema, auth.validateInput, auth.verifyUserToken, async (req,res)=>{
    if(!(await userRepo.checkID(req.user.ID)))
    return res.status(401).send('You are not authorize')
    const result=await userRepo.deleteUserAsync(req.user.ID)
    result.affectedRows!==0 ? res.status(202).send({message: 'Account deleted'}) : res.sendStatus(204)
})
